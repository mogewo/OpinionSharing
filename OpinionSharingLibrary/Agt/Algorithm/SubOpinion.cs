...
namespace OpinionSharing.Agt
{
    ...
    public class SubOpinion : AgentAlgorithm, IAATBasedAgent // 抜本的な改革かもしれないので、IAgentを実装するだけになるかも。
    {
    #region private メンバ
        //
        ThoughtTwin thought;
        ...
        //なるかもしれないBeliefのリスト 表示のため。ほんとは毎回生成すべきものをCacheしていると解釈。Viewに移すべきでは。少なくともAAT
        private List<double> beliefList;

    #endregion private メンバ

        public SubOpinion(double h = 0.9): base()
        {
        }
    ...
    #region 各種プロパティ
        public CandidateUpdaterSelector CandidateSelector
        {
            ...
            set
            {
                ...
                {
                    candidateSelector.CandidateChanged -= new EventHandler<CandidateEventArgs>((sender, e) =>
                    {
                        calcBeliefList();
                    });
                }
    ...
    #region イベント
        ...
        private void calcBeliefList()
        {
            ...
            if (candidateSelector.BeliefUpdater.ImportanceLevel == 0.5)
            {
                return;
            }
            ...
            do{
                goright = candidateSelector.BeliefUpdater.updateBelief(BlackWhiteSubject.White, goright);
                beliefList.Add(goright);
            } while(goright < Sigma );//&& i < 5);


            do{
                goleft = candidateSelector.BeliefUpdater.updateBelief(BlackWhiteSubject.Black, goleft);
